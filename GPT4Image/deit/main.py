...
def get_args_parser():
    ...
    parser.add_argument('--loss_type', type=str, default='clip', choices=utils.text_loss_getter.keys())
...
def main(args):
    ...
    utils.seed_training(args.seed + utils.get_rank())
    ...
    if args.text_emb is not None:
        _proj_dim = dataset_train.text_embeddings.shape[-1]
        model = create_model(args.model, pretrained=False, num_classes=args.nb_classes, 
            ...
            img_size=args.input_size, proj_type=args.proj_type, proj_dim=_proj_dim )
        print(f'\ntext_emb={args.text_emb}\nproj_type={args.proj_type}\nloss_type={args.loss_type}\nlamb={args.lamb}\ntau={args.tau}\n')
    else:
        model = create_model(
            args.model, pretrained=False, num_classes=args.nb_classes, drop_rate=args.drop,
    ...
    if args.model_ema:
        ...
        model_ema = ModelEma(
            ...
            resume='')
        # if hasattr(model_ema.ema, 'proj_layer'):
        #     delattr(model_ema.ema, 'proj_layer')
    ...
    if last_ckpt_path.exists():  # for automatic resume
        args.resume = str(last_ckpt_path)
        print('checkpoint.pth found at', args.output_dir, '\n automatic resuming ...')
    if args.resume:
