...
#
# Modified from GreenMIM by Haoqing Wang
# GreenMIM: https://github.com/LayneH/GreenMIM
#
...
class BasicLayer(nn.Module):
    ...
    """
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size,
                                 ...
                                 qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dim=96, depths=[2, 2, 6, 2],
                 ...
                 attn_drop_rate=0., drop_path_rate=0.1, norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False):
        super().__init__()
        ...
        self.norm = nn.ModuleList([norm_layer(embed_dim*k) for k in [2, 4, 8, 8]])
    ...
    def restore_gene(self, mask):
        ids_shuffle = torch.argsort(mask + (torch.arange(mask.shape[1])/(mask.shape[1]*5.)).unsqueeze(0).to(mask.device))
    ...
    def forward_features(self, x, mask):
        ...
        x = x.permute(0, 1, 3, 2, 4, 5).reshape(B, N, C)  # (B, 49*64, C)
        mask = mask.view(1, H // window_size, window_size, W // window_size, window_size)
        mask = mask.permute(0, 1, 3, 2, 4).reshape(1, N)  # (1, 49*64)
        coords = coords.view(1, H // window_size, window_size, W // window_size, window_size, 2)
        coords = coords.permute(0, 1, 3, 2, 4, 5).reshape(1, N, 2)  # (1, 49*64)
        ...
        for i, layer in enumerate(self.layers):
            ...
            latent.append(self.norm[i](x_vis))
            ids_restore.append(self.restore_gene(1.-vis_mask.float()).repeat(x.size(0), 1))
...
class ViT_Decoder(nn.Module):
    def __init__(self, inp_dim, embed_dim=256, out_dim=27, num_patches=196, depth=1, num_heads=8, mlp_ratio=4., qkv_bias=False, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        # pred head
        self.pred = nn.Linear(embed_dim, out_dim, bias=True)
        self.initialize_weights()
    ...
    def forward(self, x, ids_restore):
        ...
        x = torch.gather(x, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # (B, 49*w*w, D)
        x = x.reshape(x.size(0), 7, 7, int(x.size(1)**0.5/7), int(x.size(1)**0.5/7), -1).transpose(2, 3).reshape(x.shape)
...
class MaskedAutoencoderSwin(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., decoder_embed_dim=256, decoder_depth=1, decoder_num_heads=16,
                 norm_layer=nn.LayerNorm, target='HOG', hog_nbins=9, hog_bias=False, norm_pix_loss=True, **kwargs):
        super().__init__()
        # MAE encoder specifics
        self.encoder = SwinTransformer(img_size=img_size, patch_size=patch_size, in_chans=in_chans, num_classes=0, embed_dim=embed_dim, depths=depths,
                                       num_heads=num_heads, window_size=window_size, norm_layer=norm_layer, **kwargs)
        ...
        self.decoder = nn.ModuleList([
            ViT_Decoder(embed_dim*s, decoder_embed_dim//k, in_chans*hog_nbins if target == 'HOG' else in_chans*(patch_size*s)**2,
                        num_patches*t, decoder_depth, decoder_num_heads, mlp_ratio, True, norm_layer)
            for s, k, t in zip([2, 4, 8, 8], [2, 1, 1, 1], [16, 4, 1, 1])])

        # target
        if target == 'HOG':
            self.hog_enc = nn.ModuleList([HOGLayer(nbins=hog_nbins, pool=k, bias=hog_bias) for k in [8, 16, 32, 32]])
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        mask = torch.randperm(self.num_patches) <= int(mask_ratio * self.num_patches)
        mask = mask.unsqueeze(0).repeat(x.shape[0], 1).to(x.device)

        # L -> L_vis
        latent, ids_restore = self.encoder(x, mask.bool())
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        """
        if self.target == 'HOG':
            target = [self.HOG(imgs, k) for k in range(len(pred))]
        else:
            target = [self.patchify(imgs, p) for p in [8, 16, 32, 32]]
            if self.norm_pix_loss:
                target = [self.norm_pixel(tg) for tg in target]
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        pred = [self.decoder[i](latent[i], ids_restore[i]) for i in range(len(self.decoder))]
        loss = self.forward_loss(imgs, pred, mask)
        return loss, pred, mask


def mim_swin_base_patch4_win7(**kwargs):
...
def mim_swin_base_patch4_win14(**kwargs):
    model = MaskedAutoencoderSwin(
                # encoder configs
                img_size=224,
                patch_size=4,
...
def mim_swin_large_patch4_win14(**kwargs):
    model = MaskedAutoencoderSwin(
                # encoder configs
                img_size=224,
                patch_size=4,
