...
class SwinTransformer(nn.Module):
    ...
    Args:
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False
    """
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths, num_heads, window_size=7, mlp_ratio=4.,
                 num_classes=1000, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False, norm_before_mlp='ln', **kwargs):
        super().__init__()
...
def swin_base_patch4_win7(**kwargs):
...
def swin_base_patch4_win14(**kwargs):
    model = SwinTransformer(
                img_size=224,
                patch_size=4,
                in_chans=3,
...
def swin_large_patch4_win14(**kwargs):
    model = SwinTransformer(
                img_size=224,
                patch_size=4,
                in_chans=3,
