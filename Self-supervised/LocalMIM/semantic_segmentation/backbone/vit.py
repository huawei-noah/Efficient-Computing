...
#
# Modified from BEiT by Haoqing Wang
# BEiT: https://github.com/facebookresearch/mae
#
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        if patch_size == 16:
            ...
            self.fpn4 = nn.AvgPool2d(kernel_size=2, stride=2)

        elif patch_size == 8:
            self.fpn1 = nn.Sequential(nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2))
            self.fpn2 = nn.Identity()
            self.fpn3 = nn.Sequential(nn.AvgPool2d(kernel_size=2, stride=2))
            self.fpn4 = nn.Sequential(nn.AvgPool2d(kernel_size=4, stride=4))
