...
class ImageNetPairedDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            for classname in sorted(os.listdir(self.gt_folder)):
                self.paths += sorted(list(scandir(osp.join(self.gt_folder, classname), full_path=True)))
            print(f'1000CLASS IMAGENET num samples: {len(self.paths)}')
